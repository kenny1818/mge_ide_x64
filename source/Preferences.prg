...
loadideini( cinifile ,acwValues ,cSect ,cSect1 )  //hmgside.prg
   IF ! File( cIniFile )
      BEGIN INI File cIniFile
        ...
        SET SECTION cSect ENTRY "wHmgFolder"                TO d + ':\minigui'
        SET SECTION cSect ENTRY "wHmgMingw32Folder"         TO 'c:\hmg\mingw32'
        SET SECTION cSect ENTRY "wHmgMingw32HarbourFolder"  TO 'c:\hmg\harbour'
        SET SECTION cSect ENTRY "wHmgXHarbourFolder"        TO 'c:\hmg\xharbour'
        SET SECTION cSect ENTRY "wHmgEditorFolder"          TO 'notepad.exe'
        ...
        SET SECTION 'USERFLAG' ENTRY "Pflag" TO '' //aextdat[ 1 ]
        SET SECTION 'USERFLAG' ENTRY "Cflag" TO '' //aextdat[ 2 ]
        SET SECTION 'USERFLAG' ENTRY "LibsearchPath" TO '' //aextdat[ 3 ]
        SET SECTION 'USERFLAG' ENTRY "INCsearchPath" TO '' //aextdat[ 4 ]
      END INI
   ENDIF
...
if aData[ _MINGW32 ] == '3' ; Preferences.RadioGroup_2.Value := 3 ; endif
   FillAddPreferences()
   iniuserpref()
RETURN
...
*------------------------------------------------------------*
   ...
   IF isWindowActive( PREFERENCES )
      ...
      aData[ _HMG2EDITORFOLDER ]        := Preferences.Text_11.Value
      ...
      aData[ _MINGW32 ]         := iif( Preferences.RadioGroup_2.Value = 2, ".T.", ;
        str( Preferences.RadioGroup_2.Value ,1 ,0 ) ) //".F." ) // ming
...
*------------------------------------------------------------*
   LOCAL AddFile AS STRING := GetFile( { { '*.exe', '*.exe' } }, 'Editor', ProjectFolder, .F., .T. )
...
*------------------------------------------------------------*
   ...
   LOCAL cDirDef AS STRING := "c:\borland\bcc58"
...
RETURN cDirDef
********************
static proc iniuserpref()
Preferences.Txt_pflag.Value := aextdat[ 1 ]
Preferences.Txt_cflag.Value := aextdat[ 2 ]
Preferences.Txt_lib.Value := aextdat[ 3 ]
Preferences.Txt_inc.Value := aextdat[ 4 ]
retu
***********************
static proc loaduserpref()
GET aextdat[ 1 ] SECTION 'USERFLAG' ENTRY "Pflag" DEFAULT ""
GET aextdat[ 2 ] SECTION 'USERFLAG' ENTRY "Cflag" DEFAULT ""
GET aextdat[ 3 ] SECTION 'USERFLAG' ENTRY "LibsearchPath" DEFAULT ""
GET aextdat[ 4 ] SECTION 'USERFLAG' ENTRY "INCsearchPath" DEFAULT ""
retu
**********************
static proc saveuserpref( lloadfmg )
if lloadfmg
  aextdat[ 1 ] := Preferences.Txt_pflag.Value
  aextdat[ 2 ] := Preferences.Txt_cflag.Value 
  aextdat[ 3 ] := Preferences.Txt_lib.Value 
  aextdat[ 4 ] := Preferences.Txt_inc.Value
endif
SET SECTION 'USERFLAG' ENTRY "Pflag" TO aextdat[ 1 ]
SET SECTION 'USERFLAG' ENTRY "Cflag" TO aextdat[ 2 ]
SET SECTION 'USERFLAG' ENTRY "LibsearchPath" TO aextdat[ 3 ]
