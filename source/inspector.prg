...
*------------------------------------------------------------*
   ...
   ***********
   aWrk := { "AutoRelease","Break","Focused" ,"HelpButton","MaxButton","MinButton","Sizable","SysMenu","TitleBar"       ,;
             ...
             "Marquee","Checked","LeftCheck","Nominus","AdjustImage","MultiTab","MultiColumn","NoSortHeaders"           ,;
             "PaintDoubleBuffer","AllowSort", "VCenterAlign" ;
            }
...
*********
   ...
   aWrk := {"Cursor","GripperText","Icon","NotifyIcon","NotifyTooltip","Title","Caption","HelpId","Name","Picture","ToolTip";
           ...
           ,"PageToolTips","CheckboxItem","LockColumns" }
   ...
   *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   IF A1 = "Picture" .AND. xTypeControl( A3 ) = "BROWSE"
      cnt_s := 1
   ENDIF
   *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   IF cnt_s > 0
      ...
      IF A1 == "Picture" .AND. xTypeControl( A3 ) = "BROWSE"
         xGridPropTxt.Button_3.Visible := .F.
      ENDIF
   ...
   *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   IF A1 = "WindowType"
      ...
      xGridPropWin.Combo_1.Value := aScan({"CHILD","MAIN ","MODAL","SPLITCHILD","STANDARD","PANEL","MDI ","MAINMDI","MDICHILD" }, A2 )
...
*------------------------------------------------------------*
   ...
   if xTypeControl( ControlName ) == 'LABEL'
    ***********************************
    z            := GetControlIndex( ControlName, "Form_1" )
    xRow         := _HMG_aControlContainerRow[ z ]
...
*------------------------------------------------------------*
   LOCAL aWrk AS ARRAY := {"CHILD","MAIN","MODAL","SPLITCHILD","STANDARD","PANEL","MDI","MAINMDI","MDICHILD"} // // kenny ooi 20210327 {"CHILD","MAIN","MODAL","SPLITCHILD","STANDARD","MDI","MAINMDI","MDICHILD"}
   local A3   AS USUAL //? VarType
...
//msginfo( { a3 ,CRLF ,aWrk[ xGridPropWin.Combo_1.Value ] ,CRLF ,a1 } )
...
*------------------------------------------------------------*
   ...
   *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   IF A3 # "Form"
      ...
      *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      IF ( A1 = "Picture" .OR. A1 = "Icon" ) .AND. xTypeControl( A3 ) # "GETBOX" .AND. xTypeControl( A3 ) # "BROWSE" .OR. ;
   ...
   *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   IF A3 # "Form"
      ...
      *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      IF A1 = "VCenterAlign" .AND. xTypeControl( A3 ) = "LABEL"
         SetProperty( "FORM_1", A3, "VISIBLE", .F. )
         if A4 == ".T."
            SetProperty( "FORM_1", A3, "Alignment", "VCENTER" )
         ELSE
            ChangeStyle ( GetControlHandle( A3, "FORM_1" ) , , SS_CENTERIMAGE )
         ENDIF
         SetProperty( "FORM_1", A3, "VISIBLE", .T. )
      ENDIF
