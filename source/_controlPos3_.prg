...
#else
     nId = ( LONG_PTR ) hb_codeblockId( bl ) ;
#endif
     HB_RETNL( nId );
 }
 else
...
{
  LONG Style = GetWindowLongPtr( (HWND) HB_PARNL(1), GWL_STYLE );
   if (hb_parl(3))
   {
    SetWindowLongPtr( (HWND) HB_PARNL(1), GWL_STYLE, Style | hb_parni(2) );
   }
   else    //reset bits
   {
    SetWindowLongPtr( (HWND) HB_PARNL(1), GWL_STYLE, Style & (~hb_parni(2)) );
...
LRESULT CALLBACK MyFormProc (HWND hWin, UINT Msg, WPARAM wParam, LPARAM lParam)
{
        ...
        switch( Msg )
        {
                case WM_DESTROY:
                        ...
                        SetWindowLongPtr( hWin, GWLP_WNDPROC, ( LONG_PTR )OldWndProc );
...
HB_FUNC ( _CONTROLPOS_C_SETFORMPROC_ )
{
   ...
   if( hb_parl(2) )
   {
     SetProp(  hWin, "oldformproc", (HWND) GetWindowLongPtr(  hWin, GWLP_WNDPROC ) ) ;
     SetWindowLongPtr( hWin, GWLP_WNDPROC, ( LONG_PTR )(WNDPROC)MyFormProc ) ;
   }
   else
   {
      OldWndProc = (WNDPROC)GetProp( hWin, "oldformproc" );
